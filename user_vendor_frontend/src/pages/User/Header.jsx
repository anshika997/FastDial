...
import SignupModal from "../../pages/User/Login/SignupModal";
...
const Header = () => {
  ...
  const [formDataFromProfile, setFormDataFromProfile] = useState(null);

  const handleFocus = () => setShowRecentSearches(true);
  const handleClose = () => setShowRecentSearches(false);

  // Re-check token on every route change (covers same-tab login)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // 'storage' fires from other tabs; 'tokenChanged' is our custom same-tab event
