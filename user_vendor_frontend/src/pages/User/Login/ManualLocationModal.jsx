...
const ManualLocationModal = ({ isOpen, onClose, onOpenAutomaticLocationModal, setFormData, formData }) => {
  ...
  const handleComplete = () => {
    ...
    if (!token) {
      setLocalError("Please login first before completing your profile.");
      return;
    }
    if (!formData?.customer_address?.city || !formData?.customer_address?.state) {
      setLocalError("Please select a valid location from the search results.");
      return;
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white rounded-lg shadow-lg max-w-md w-full p-8 flex flex-col gap-5 relative">
        ...
        {formData?.customer_country && <div
          className={`flex flex-col gap-1 cursor-pointer p-3 rounded-md transition-colors ${registerLoading ? 'bg-blue-50 opacity-70' : 'hover:bg-gray-50'}`}
          onClick={!registerLoading ? handleComplete : undefined}
        >
          <div className="flex items-center gap-2 font-semibold text-gray-900">
            <img src={MapPinG} alt="Location Icon" className="w-5 h-5" />
            {formData?.customer_address.city}
            {registerLoading && <span className="ml-2 text-blue-500 text-sm">Saving...</span>}
          </div>
