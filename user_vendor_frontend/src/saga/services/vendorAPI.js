...
export const vendorVerifyEmailOtpAPI = async (vendor_email, otp) => {
  const response = await api.post('/vendors/verify-email-otp', { vendor_email, otp });
  return response.data;
};

export const vendorResendEmailOtpAPI = async (vendor_email) => {
  const response = await api.post('/vendors/resend-email-otp', { vendor_email });
  return response.data;
};
