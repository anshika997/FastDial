...
function* loginCustomerSaga(action) {
  try {
    const { customer_email, password } = action.payload;
    console.log('loginCustomerSaga: Attempting login with:', { customer_email });
    const response = yield call(loginCustomerAPI, customer_email, password);
    ...
    // Notify same-tab listeners (Header) that token changed
    ...
    if (error.response?.data?.requiresVerification) {
      yield put(signupSuccess({ email: error.response.data.email }));
    } else {
...
function* signupCustomerSaga(action) {
  try {
    const { customer_name, customer_email, mobile, password } = action.payload;
    console.log('signupCustomerSaga: Attempting signup with:', { customer_name, customer_email, mobile });
    const response = yield call(signupCustomerAPI, customer_name, customer_email, mobile, password);
    console.log('signupCustomerSaga: API Response:', response.data);

    // Signup now requires OTP verification — do NOT auto-login
    yield put(signupSuccess({ email: customer_email }));
    console.log('signupCustomerSaga: signupSuccess dispatched, waiting for OTP verification');
  } catch (error) {
...
function* verifyOtpSaga(action) {
  try {
    const { customer_email, otp } = action.payload;
    console.log('verifyOtpSaga: Verifying OTP for:', customer_email);
    const response = yield call(verifyOtpAPI, customer_email, otp);
    console.log('verifyOtpSaga: API Response:', response.data);
    ...
    yield put(loginSuccess({
      user: { customer_email, customer_id: customerId },
      token,
      new_user: true,
    }));
    console.log('verifyOtpSaga: Verification successful, user logged in');
  } catch (error) {
    console.error('verifyOtpSaga: Error:', error.response?.data || error.message);
...
function* resendOtpSaga(action) {
  try {
    const { customer_email } = action.payload;
    console.log('resendOtpSaga: Resending OTP for:', customer_email);
    const response = yield call(resendOtpAPI, customer_email);
    console.log('resendOtpSaga: API Response:', response.data);
    yield put(resendOtpSuccess());
  } catch (error) {
    console.error('resendOtpSaga: Error:', error.response?.data || error.message);
    yield put(resendOtpFailure(error.response?.data?.message || error.message));
  }
}
...
function* getCustomerServiceDetailsSaga() {
  ...
  try {
    const token = localStorage.getItem('token');
    console.log('getCustomerServiceDetailsSaga: Token from localStorage:', token);
    if (!token) {
      throw new Error('Please Login to continue');
    }
    ...
    console.log('getCustomerServiceDetailsSaga: Bookings with scheduled_date:', response.data.filter(b => b.scheduled_date));
    yield put(getCustomerServiceDetailsSuccess(response.data));
    console.log('getCustomerServiceDetailsSaga: getCustomerServiceDetailsSuccess dispatched');
  } catch (error) {
