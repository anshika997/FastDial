...
export default function AffidavitsRequest() {
  ...
  const handleSubmit = async () => {
    if (!formData.tds_statement_url) {
      setError("TDS Statement File are required.");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      const tds_statement_url = await uploadFile(formData.tds_statement_url);
      ...
      const response = await axios.post(
        `${BASEURL}/data/insertTdsReturns`, // Updated API endpoint
        finalData,
        {
          headers: {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      <h1 className="text-xl font-bold text-center mb-4">
        Insert Tds Returns Form
      </h1>
      {error && <div className="text-red-500 mb-4">{error}</div>}
      <div className="lg:grid lg:grid-cols-3 gap-4 mb-4 bg-white mx-10 p-5 rounded-lg">
        <div className="mb-4">
          <label>Tds Statement (Upload file)</label>
          <div className="flex items-center gap-2">
            <input
              type="file"
              name="tds_statement_url"
              onChange={handleChange}
              className="w-full p-2 border rounded"
            />
            <button
              type="button"
              onClick={() => previewFile(formData.tds_statement_url)}
        ...
        <div className="mb-4">
          <label>Any Statement</label>
          <div className="flex items-center gap-2">
            <input
              type="text"
              name="any_statement_id"
              onChange={handleChange}
              className="w-full p-2 border rounded"
            />
            <button
              type="button"
              onClick={() => previewFile(formData.any_statement_id)}
              className="text-blue-500"
            >
              <FaEye />
            </button>
