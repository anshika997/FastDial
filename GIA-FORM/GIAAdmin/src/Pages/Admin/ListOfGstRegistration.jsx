...
export default function ListOfPropertyRegistration() {
  ...
  function fetchData() {
    axios
      .get(`${BASEURL}/data/getGstRegistrations`, {
        headers: {
    ...
    axios
      .get(
        `${BASEURL}/data/getVendors?vendor_bussiness_name=GstRegistrations`,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
  ...
  const handleApprove = (id) => {
    ...
    axios
      .post(
        `${BASEURL}/data/updateGstRegistrations`,
        {
          id: id,
          is_approved: 1,
  ...
  const handleSubmitRejection = () => {
    ...
    axios
      .post(`${BASEURL}/data/updateGstRegistrations`, payload, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      })
        ...
        <div className="bg-white p-4 rounded-lg shadow w-full">
          ...
          <div className="w-full overflow-x-auto">
            <table className="min-w-max w-full border-collapse">
              <thead>
                <tr className="bg-gray-200 text-md text-center">
                  <th className="p-2">ID</th>
                  <th className="p-2">Pan No</th>
                  <th className="p-2">Vendor ID</th>
                  <th className="p-2">Assigned Vendor ID</th>
              ...
              <tbody>
                {filteredData.length > 0 ? (
                  filteredData.map((item, index) => (
                    <tr key={index} className="border-b text-md text-center">
                      <td className="p-2">{item.id}</td>
                      <td className="p-2">
                        {item.pan_no ? item.pan_no : "NA"}
                      </td>
                      <td className="p-2">
                        {item.vendor_id ? item.vendor_id : "NA"}
                      </td>
                      <td className="p-2">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center w-full">
          <div className="bg-white rounded-lg p-6 w-[95%] lg:w-3/4  shadow-lg relative  h-80 md:h-88 overflow-y-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div>
                <p>
                  <strong>Pan No:</strong> {selectedData.pan_no || "N/A"}
                </p>
              </div>
              ...
              <div>
                <p>
                  <strong>Assigned Vendor Date:</strong>{" "}
                  {new Date(
                    selectedData.assigned_vendor_date_time
                  ).toLocaleDateString()}
                </p>
              </div>
              <div>
                <p>
                  <strong>Vendor Accepted Date:</strong>{" "}
                  {new Date(
                    selectedData.vendor_accepted_date_time
                  ).toLocaleDateString()}
                </p>
              </div>
              <div>
              ...
              <div>
                <p>
                  <strong>Vendor Rejected Reason:</strong>{" "}
                  {selectedData.vendor_rejected_reason || "N/A"}
                </p>
              </div>
              <div>
                <p>
                  <strong>Vendor Completed Date:</strong>{" "}
                  {new Date(
                    selectedData.vendor_completed_date_time
                  ).toLocaleDateString()}
                </p>
              </div>
              ...
              {[
                ...
                { label: "Electricity Bill", field: "electricity_bill_url" },
                { label: "rental Agreement", field: "rental_agreement_url" },
                { label: "director Photo", field: "director_photo_url" },
                { label: "director Pan", field: "director_pan_url" },
                { label: "director Aadhar", field: "director_aadhar_url" },
              ].map(({ label, field }) => (
                <div key={field}>
                  <p>
                    <strong>{label}:</strong>
              ...
              {/* Rejected By */}
              {(!selectedData?.assigned_vendor || !item?.rejected_by) && (
                <div>
                  <p>
                    <strong>Rejected By:</strong>{" "}
                    {selectedData.rejected_by || "N/A"}
                  </p>
                </div>
              )}
              {/* Rejected Reason */}
              {(!selectedData.assigned_vendor || !item.rejected_by) && (
                <div>
                  <p>
                    ...
                    {selectedData.rejected_reason || "N/A"}
                  </p>
                </div>
              )}
            </div>
