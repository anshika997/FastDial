...
export default function AffidavitsRequest() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const pan_url = await uploadFile(formData.pan_url);
      const dsc_url = await uploadFile(formData.dsc_url);
      const authorised_signatory_url = await uploadFile(
        formData.authorised_signatory_url
      );

      const finalData = {
        ...formData,
        pan_url,
        authorised_signatory_url,
        dsc_url,

      };

      const response = await axios.post(
        `${BASEURL}/data/insertTanRegistrations`, // Updated API endpoint
        finalData,
        {
          headers: {
      ...
      <div className="lg:grid lg:grid-cols-3 gap-4 mb-4 bg-white mx-10 p-5 rounded-lg">
        <div className="mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => previewFile(formData.pan_url)}
        ...
        <div className="mb-4">
          <label>Authorised Signatory (Upload file)</label>
          <div className="flex items-center gap-2">
            <input
              type="file"
              name="authorised_signatory_url"
              onChange={handleChange}
              className="w-full p-2 border rounded"
            />
            <button
              type="button"
              onClick={() => previewFile(formData.authorised_signatory_url)}
              className="text-blue-500"
            >
              <FaEye />
            </button>
        ...
        <div className="mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => previewFile(formData.dsc_url)}
              className="text-blue-500"
            >
              <FaEye />
            </button>
