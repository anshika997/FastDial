...
exports.loginRequest = catchAsyncError(async (req, res, next) => {
  ...
  const searchUserSql = `SELECT * FROM class_admins WHERE email = ? `;
...
exports.loginVerify = catchAsyncError(async (req, res, next) => {
  ...
  if (otpRes) {
    const searchUserSql = `SELECT * FROM class_admins WHERE admin_mobile = ? `;
    let searchResult = await db(searchUserSql, phone);
    const userdetails = searchResult[0];

    createAndSendToken(userdetails, 200, req, res);
  } else {
...
exports.signUp = catchAsyncError(async (req, res, next) => {
  const { admin_name, admin_mobile, email, password } = req.body;
  const err = validationResult(req);
  if (!err.isEmpty()) {
    return res
      .status(403)
  ...
  const searchUserSql = `SELECT * FROM class_admins WHERE admin_mobile = ? OR email = ?`;
  let searchResult = await db(searchUserSql, [admin_mobile, email]);
  if (searchResult.length > 0) {
...
exports.protect = catchAsyncError(async (req, res, next) => {
  ...
  // const searchUserSql = `SELECT * FROM class_admins WHERE admin_id = ?`;
