...
exports.loginRequest = catchAsyncError(async (req, res, next) => {
  ...
  const searchUserSql = `SELECT * FROM vendors WHERE email = ? `;
...
exports.loginVerify = catchAsyncError(async (req, res, next) => {
  ...
  if (otpRes) {
    const searchUserSql = `SELECT * FROM vendors WHERE vendor_number = ? `;
    let searchResult = await db(searchUserSql, phone);
...
exports.signUp = catchAsyncError(async (req, res, next) => {
  const { email, password, vendor_number } = req.body;
  ...
  const searchUserSql = `SELECT * FROM vendors WHERE vendor_number = ? OR email = ?`;
  let searchResult = await db(searchUserSql, [vendor_number, email]);
  if (searchResult.length > 0) {
    return next(new AppError("Number or email already registered", 400));
  }
  const salt = await bcrypt.genSalt(10);
  req.body.password = await bcrypt.hash(password, salt);
  await generateInsertStatement("vendors", req);
  res.status(200).json({
    success: "true",
  });
});
