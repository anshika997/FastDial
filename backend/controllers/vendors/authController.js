...
exports.insertVendor = catchAsyncError(async (req, res, next) => {
  ...
  await db(
    "UPDATE VENDORS SET email_otp = ?, otp_expires_at = ?, email_verified = 0 WHERE vendor_id = ?",
    [otp, otpExpiresAt, vendorId]
  );

  console.log("Vendor created with ID:", vendorId, "OTP:", otp);

  try {
    await sendOTPEmail(vendor_email, otp, "vendor");
  } catch (err) {
...
// ============================================================
// VERIFY EMAIL OTP — Vendor email verification
// ============================================================
exports.verifyEmailOtp = catchAsyncError(async (req, res, next) => {
  const { vendor_email, otp } = req.body;
  ...
  await db(
    "UPDATE VENDORS SET email_verified = 1, email_otp = NULL, otp_expires_at = NULL WHERE vendor_id = ?",
    [vendor.vendor_id]
  );
  ...
  console.log("Vendor email verified for:", vendor.vendor_id);
...
// ============================================================
// RESEND EMAIL OTP — Vendor email OTP resend
// ============================================================
exports.resendEmailOtp = catchAsyncError(async (req, res, next) => {
  ...
  await db(
    "UPDATE VENDORS SET email_otp = ?, otp_expires_at = ? WHERE vendor_id = ?",
    [otp, otpExpiresAt, vendor.vendor_id]
  );

  console.log("Resend OTP for vendor:", vendor.vendor_id, "OTP:", otp);
