...
exports.getcustomerservice = catchAsyncError(async (req, res, next) => {
  getData(req, res, "CUSTOMER_SERVICE_DETAILS");
});
...
exports.getAllVendorsWithServices = async (req, res) => {
  const query = `
    ...
    WHERE 
      -- ✅ Vendor has active free trial
      EXISTS (
        SELECT 1 
        FROM VENDOR_SUBSCRIPTIONS vsub
        WHERE vsub.vendor_id = v.vendor_id
          AND vsub.status = 'active'
      )
      OR
      -- ✅ Vendor has a valid paid subscription (renewal_date >= today)
      EXISTS (
        SELECT 1 
        FROM SUBSCRIPTIONPAYMENTSBYVENDOR spv
        WHERE spv.vendor_id = v.vendor_id
          AND JSON_UNQUOTE(JSON_EXTRACT(spv.payment_details, '$.renewal_date')) >= CURDATE()
      )
  ...
  try {
    ...
    res.status(200).json({
      ...
      addresses: addresses.map((addr) => ({
        ...
        ...addr.address, // no JSON.parse needed
      })),
    });
  } catch (error) {
